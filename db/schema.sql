...
CREATE TABLE reply (
    ...
    memberId INT(10) UNSIGNED NOT NULL,
    relTypeCode CHAR(30) NOT NULL COMMENT '관련데이터타입코드',
    articleId INT(10) UNSIGNED NOT NULL COMMENT '관련데이터번호',
    `body` TEXT NOT NULL
);
...
# 댓글 테이블에 인덱스 걸기
ALTER TABLE `reply` ADD INDEX (`relTypeCode` , `articleId`);

UPDATE reply SET updateDate = NOW(), `body` = 'sasdadasfaaa' WHERE memberId = 
2 AND articleId = 2 AND id=35 